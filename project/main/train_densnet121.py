...
def main():
    ...
    criterion = LMFLoss(cls_num_list, device, weight=class_weights,
                       alpha=alpha, beta=beta, gamma=gamma, max_m=max_m, s=s, add_LDAM_weigth=True)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        logger.info(f"Epoch {epoch:02d} LR: {current_lr:.6f} | Train Loss: {train_loss:.4f} | Train Acc: {train_acc:.4f}")
        ...
        logger.info(
            f"Epoch {epoch:02d} | Val Loss: {val_loss:.4f} | Val Acc: {val_acc:.4f} | "
            f"Val Precision (per class): [{prec_str}] | Val Recall (per class): [{rec_str}] | Val Macro F1: {val_macro_f1:.4f}"
        )
